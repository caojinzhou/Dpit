...
# Then edit the variable "zeta_function_filename" in this file,
...
#zeta_func1 <- "/usr/local/bin/zeta_func1"
### EDIT THIS LOCATION!!! #### 
...
dzeta <- function(x, threshold=1, exponent, log=FALSE) {
  C <- zeta_func(exponent,threshold) #C<-as.numeric(C)#HJ: Seems optim's SANN requires here a num. vector, not dataframe.
  if (log) {
...
pzeta <- function(x, threshold=1, exponent, lower.tail=TRUE, log.p=FALSE) {
  C <- zeta_func(exponent,threshold) #C<-as.numeric(C)#HJ
...
zeta.loglike <- function(x,threshold=1,exponent) {
  L <- sum(suppressWarnings(dzeta(x,threshold,exponent,log=TRUE)))
  return(L)
}
...
zeta.fit.direct <- function(x, threshold) {
  alpha_0 <- zeta.fit.approx(x,threshold)   # Use approximate method to start optimization
  negloglike <- function(alpha) {-zeta.loglike(x,threshold,alpha)}
  est <- suppressWarnings(nlm(f=negloglike,p=alpha_0))
  #est<-suppressWarnings(optim(par=alpha_0,fn=negloglike,method="SANN"))
  return(est$estimate[1]) # Extract and return the parameter
  #return(est$par[1])
}
...
zeta_func_once.gsl <- function(q,s) {
  # zeta.command <- paste(system.file(package="Dpit", "zeta_func"), s,q)
