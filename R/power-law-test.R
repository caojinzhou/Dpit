...
vuong <- function(x) {
	...
	list(loglike.ratio=R,Vuong=v,p.two.sided=p2) #I just need these three output variables
}
...
pareto.exp.llr <- function(x,pareto.d,exp.d) {
	...
	suppressWarnings(dpareto(x,threshold=xmin,exponent=alpha,log=TRUE)) - suppressWarnings(dexp(x,lambda,log=TRUE)) + suppressWarnings(pexp(xmin,lambda,lower.tail=FALSE,log.p=TRUE))
}
...
pareto.lnorm.llr <- function(x,pareto.d,lnorm.d) {
	...
	suppressWarnings(dpareto(x,threshold=xmin,exponent=alpha,log=TRUE)) - suppressWarnings(dlnorm(x,meanlog=m,sdlog=s,log=TRUE)) + suppressWarnings(plnorm(xmin,meanlog=m,sdlog=s,lower.tail=FALSE,log.p=TRUE))
}
...
pareto.weibull.llr <- function(x,pareto.d,weibull.d) {
	...
	suppressWarnings(dpareto(x,threshold=xmin,exponent=alpha,log=TRUE)) - suppressWarnings(dweibull(x,shape=shape,scale=scale,log=TRUE)) + suppressWarnings(pweibull(xmin,shape=shape,scale=scale,lower.tail=FALSE,log.p=TRUE))
}
...
##Harry: I think the original author meant zeta.R and discexp.R
zeta.exp.llr <- function(x,zeta.d,exp.d) {
  xmin <- zeta.d$threshold
  alpha <- zeta.d$exponent
  lambda <- exp.d$lambda
  x <- x[x>=xmin]
  suppressWarnings(dzeta(x,xmin,alpha,log=TRUE)) - suppressWarnings(ddiscexp(x,lambda,xmin,log=TRUE))
}
...
zeta.lnorm.llr <- function(x,zeta.d,lnorm.d) {
  ...
  sdlog <- lnorm.d$sdlog
  x <- x[x>=xmin]
  suppressWarnings(dzeta(x,xmin,alpha,log=TRUE)) - dlnorm.tail.disc(x,meanlog,sdlog,xmin,log=TRUE)
}
...
zeta.poisson.llr <- function(x,zeta.d,pois.d) {
  ...
  rate <- pois.d$rate
  x <- x[x>=xmin]
  suppressWarnings(dzeta(x,threshold=xmin,exponent=alpha,log=TRUE)) - dpois.tail(x,threshold=xmin,rate=rate,log=TRUE)
}
...
zeta.weib.llr <- function(x,zeta.d,weib.d) {
   ...
   scale <- weib.d$scale
   x <- x[x>=xmin]
   suppressWarnings(dzeta(x,xmin,alpha,log=TRUE)) - suppressWarnings(ddiscweib(x,shape,scale,xmin,log=TRUE))
}
...
#  x <- x[x>=xmin]
#  suppressWarnings(dzeta(x,threshold=xmin,exponent=alpha,log=TRUE)) - suppressWarnings(dyule(x,beta,xmin,log=TRUE))
#}
