...
lnorm.fit <- function(x,threshold=0,method="tail") { 
  switch(method,
   tail = {
    ...
    else { fit <- lnorm.fit.moments(x) } ####HJ: Why no treshold here? There's one below. Mistake?
  },
  shift = {
...
lnorm.fit.max <- function(x, threshold = 0) {
  ...
  fit <- list(type="lnorm",meanlog=est$par[1],sdlog=est$par[2],
              datapoints.over.threshold = length(x), loglike=-est$value)
...
lnorm.loglike.tail <- function(x, mlog, sdlog, threshold = 0) {
  ...
  ThresholdProb <- suppressWarnings(plnorm(threshold, mlog, sdlog, log.p=TRUE, lower.tail=FALSE))
...
lnorm.loglike.shift <- function(x, mlog, sdlog, x0=0) {
  ...
  x <- x[x>=x0]
  x <- x-x0
  L <- sum(suppressWarnings(dlnorm(x,mlog,sdlog,log=TRUE)))
  return(L)
}
...
dlnorm.tail <- function(x, meanlog, sdlog, threshold=0,log=FALSE) {
  ...
  if (log) {  
    f <- function(x) {suppressWarnings(dlnorm(x,meanlog,sdlog,log=TRUE)) - suppressWarnings(plnorm(threshold,meanlog,sdlog,log.p=TRUE))}
  } else {
    f <- function(x) {suppressWarnings(dlnorm(x,meanlog,sdlog))/suppressWarnings(plnorm(threshold,meanlog,sdlog))}
  }
...
plnorm.tail <- function(x,meanlog,sdlog,threshold=0,lower.tail=TRUE,log.p=FALSE) {
  c <- suppressWarnings(plnorm(threshold,meanlog,sdlog,lower.tail=FALSE))
  c.log <- suppressWarnings(plnorm(threshold,meanlog,sdlog,lower.tail=FALSE,log.p=TRUE))
  if ((!lower.tail) && (!log.p)) {
    f <- function(x) {suppressWarnings(plnorm(x,meanlog,sdlog,lower.tail=FALSE))/c}
  }
  if ((lower.tail) && (!log.p)) {
    f <- function(x) {1 - suppressWarnings(plnorm(x,meanlog,sdlog,lower.tail=FALSE))/c}
  }
  if ((!lower.tail) && (log.p)) {
    f <- function(x) {suppressWarnings(plnorm(x,meanlog,sdlog,lower.tail=FALSE,log.p=TRUE)) - c.log}
  }
  if ((lower.tail) && (log.p)) {
    f <- function(x) {log(1 - suppressWarnings(plnorm(x,meanlog,sdlog,lower.tail=FALSE))/c)}
  }
